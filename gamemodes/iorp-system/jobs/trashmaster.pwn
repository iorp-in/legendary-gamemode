...
stock CanUseTrashExport(playerid) {
    return IsTimePassedForPlayer(playerid, "IntervalForResellingUsage", 5 * 60);
}

hook GlobalOneMinuteInterval() {
    TRASH_BAG_VALUE = Random(150, 350);
    return 1;
}
...
hook OnGameModeInit() {
    new label[128];
    for (new i; i < sizeof(TrashData); i++) {
        format(label, sizeof(label), "%s\n{FFFFFF}\npress Y to collect.", (TrashData[i][TrashType] == TYPE_BIN) ? ("Trash Bin") : ("Dumpster"));
        TrashData[i][TrashLabel] = CreateDynamic3DTextLabel(label, 0x2ECC71FF, TrashData[i][TrashX], TrashData[i][TrashY], TrashData[i][TrashZ] + 1.25, 15.0);
    ...
    for (new i; i < sizeof(FactoryData); i++) {
        ...
        FactoryData[i][FactoryLabel] = CreateDynamic3DTextLabel(label, 0x2ECC71FF, FactoryData[i][FactoryX], FactoryData[i][FactoryY], FactoryData[i][FactoryZ] + 0.5, 15.0);
    ...
    if (newstate == PLAYER_STATE_DRIVER) {
        ...
        if (GetVehicleModel(vehicleid) == 408) {
            if (LoadedTrash[vehicleid] > 0) {
                new string[128];
                format(string, sizeof(string), "TRASHMASTER JOB: {FFFFFF}This vehicle has {F39C12}%d {FFFFFF}trash bags which is worth {2ECC71}$%s.", LoadedTrash[vehicleid], FormatCurrency(LoadedTrash[vehicleid] * TRASH_BAG_VALUE));
                SendClientMessageEx(playerid, 0x2ECC71FF, string);
                SendClientMessageEx(playerid, 0x2ECC71FF, "TRASHMASTER JOB: {FFFFFF}You can sell your trash bags to recycling factories marked by a truck icon.");
                ...
                SendClientMessageEx(playerid, 0x2ECC71FF, "TRASHMASTER JOB: {FFFFFF}You can collect trash and sell them at recycling factories.");
                SendClientMessageEx(playerid, 0x2ECC71FF, "TRASHMASTER JOB: {FFFFFF}Find trash cans/dumpsters and press Y.");
            }
...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys) {
    if (newkeys == KEY_YES) {
        ...
        if (id != -1) {
            if (HasTrash[playerid]) { SendClientMessageEx(playerid, -1, "{4286f4}[Error]: {FFFFEE}You're already carrying a trash bag."); return ~1; }
            if (IsPlayerInAnyVehicle(playerid)) { SendClientMessageEx(playerid, -1, "{4286f4}[Error]: {FFFFEE}You can't use this command in a vehicle."); return ~1; }
            new vehicleid = GetPVarInt(playerid, "LastVehicleID");
            if (GetVehicleModel(vehicleid) != 408) { SendClientMessageEx(playerid, -1, "{4286f4}[Error]: {FFFFEE}Your last vehicle has to be a Trashmaster."); return ~1; }
            if (TrashData[id][TrashLevel] < 1) { SendClientMessageEx(playerid, -1, "{4286f4}[Error]: {FFFFEE}There's nothing here."); return ~1; }
            new Float:x, Float:y, Float:z;
            // GetVehicleBoot(vehicleid, x, y, z);
            GetVehiclePos(vehicleid, x, y, z);
            if (GetPlayerDistanceFromPoint(playerid, x, y, z) >= 30.0) { SendClientMessageEx(playerid, -1, "{4286f4}[Error]: {FFFFEE}You're not near your Trashmaster."); return ~1; }
            ...
            Streamer_SetIntData(STREAMER_TYPE_3D_TEXT_LABEL, TrashData[id][TrashLabel], E_STREAMER_COLOR, (TrashData[id][TrashLevel] == 0) ? -1 : 0xF39C12FF);
            SendClientMessageEx(playerid, 0x2ECC71FF, "TRASHMASTER JOB: {FFFFFF}You can press {F39C12}~k~~CONVERSATION_NO~ {FFFFFF}to remove the trash bag.");
            return ~1;
        }
    }
    if ((newkeys & KEY_NO) && HasTrash[playerid]) {
        Trash_ResetPlayer(playerid);
        SendClientMessageEx(playerid, 0x2ECC71FF, "TRASHMASTER JOB: {FFFFFF}Trash bag removed.");
        return ~1;
    }
...
hook OnPlayerEnterDynamicCP(playerid, checkpointid) {
    if (checkpointid == TrashCP[playerid]) {
        if (!HasTrash[playerid]) return SendClientMessageEx(playerid, -1, "{4286f4}[Error]: {FFFFEE}You're not carrying a trash bag.");
        new vehicleid = GetPVarInt(playerid, "LastVehicleID");
        if (LoadedTrash[vehicleid] >= TRASH_LIMIT) return SendClientMessageEx(playerid, -1, "{4286f4}[Error]: {FFFFEE}This vehicle is full, you can't load any more trash.");
        LoadedTrash[vehicleid]++;
        ApplyAnimation(playerid, "GRENADE", "WEAPON_throwu", 4.1, 0, 0, 0, 0, 0);
        SendClientMessageEx(playerid, 0x2ECC71FF, "TRASHMASTER JOB: {FFFFFF}You've collected a trash bag.");
    ...
    if (GetPlayerState(playerid) == PLAYER_STATE_DRIVER) {
        for (new i; i < sizeof(FactoryData); i++) {
            if (checkpointid == FactoryData[i][FactoryCP]) {
                if (!CanUseTrashExport(playerid)) {
                    return AlexaMsg(playerid, sprintf(
                        "the recyclining factory is not available right now, try after %s",
                        UnixToHumanEx(GetLastTimeForPlayer(playerid, "IntervalForResellingUsage") + 5 * 60)
                    ));
                }
                new string[128], vehicleid = GetPlayerVehicleID(playerid), cash = LoadedTrash[vehicleid] * TRASH_BAG_VALUE;
                format(string, sizeof(string), "TRASHMASTER JOB: {FFFFFF}Sold {F39C12}%d {FFFFFF}bags of trash and earned {2ECC71}$%s.", LoadedTrash[vehicleid], FormatCurrency(cash));
                SendClientMessageEx(playerid, 0x2ECC71FF, string);
                vault:PlayerVault(playerid, cash, "earned from trashmaster job", Vault_ID_Government, -cash, sprintf("%s was paid for trashmaster job", GetPlayerNameEx(playerid)));
