...
cmd:removefaction(playerid, const params[]) {
    if (!IsPlayerMasterAdmin(playerid)) return 0;

    new Account[50];
    if (sscanf(params, "s[50]", Account)) return SendClientMessage(playerid, -1, "[USAGE]: /removefaction [Account]");
    if (!IsValidAccount(Account)) return SendClientMessage(playerid, -1, "[Alexa]: Account not Found");

    new factionid = Faction:GetAccountFaction(Account);
    if (factionid == -1) return SendClientMessage(playerid, -1, "[Alexa]: player is not in any faction");

    foreach(new targetid : Player) {
        if (IsStringSame(GetPlayerNameEx(targetid), Account)) {
            if (Faction:GetPlayerFID(targetid) == -1) return SendClientMessage(playerid, -1, "[Alexa]: player does not have any faction");
            Faction:SetPlayer(targetid, -1, -1);
            AlexaMsg(targetid, "your faction has been removed");
            return SendClientMessage(playerid, -1, sprintf("[Alexa]: %s removed from his faction", Account));
        }
    }

    Faction:SetAccount(Account, -1, -1);
    SendClientMessage(playerid, -1, sprintf("[Alexa]: %s offline removed from his faction", Account));
    return 1;
}

cmd:setfaction(playerid, const params[]) {
    if (!IsPlayerMasterAdmin(playerid)) return 0;

    new Account[50], factionid, rankid;
    if (sscanf(params, "s[50]dd", Account, factionid, rankid)) return SendClientMessage(playerid, -1, "[USAGE]: /setfaction [Account] [factionid] [rankid]");
    if (!IsValidAccount(Account)) return SendClientMessage(playerid, -1, "[Alexa]: Account not Found");
    if (!Iter_Contains(factions, factionid)) return SendClientMessage(playerid, -1, "[Alexa]: invalid factionid");

    new oFactionId = Faction:GetAccountFaction(Account);
    new oRankId = Faction:GetAccountFactionRank(Account);

    if (oFactionId == factionid && oRankId == rankid) return SendClientMessage(playerid, -1, "[Alexa]: Player is already in this faction and rank");
    if (rankid < 1 || rankid > Faction:Data[factionid][RankLimit]) return SendClientMessage(playerid, -1, "[Alexa]: invalid rankid");
    if (oFactionId != factionid && Faction:GetMemberCount(factionid) == Faction:Data[factionid][Members_Limit]) return SendClientMessage(playerid, -1, "[Alexa]: Faction Reached it's maximum Limit");

    foreach(new targetid : Player) {
        if (IsStringSame(GetPlayerNameEx(targetid), Account)) {
            if (Faction:PlayerData[targetid][FactionID] == factionid) return SendClientMessage(playerid, -1, "[Alexa]: Player Already in this Faction");
            Faction:SetPlayer(targetid, factionid, rankid);
            AlexaMsg(targetid, sprintf("your faction now {FFCC66}%s{FFFFFF} with rank {FFCC66}%s", Faction:GetName(factionid), Faction:GetRankName(factionid, rankid)));
            return SendClientMessage(playerid, -1, sprintf("[Alexa]: %s faction updated to %s [%d] with rank %s", Account, Faction:GetName(factionid), factionid, Faction:GetRankName(factionid, rankid)));
        }
    }

    Faction:SetAccount(Account, factionid, rankid);
    SendClientMessage(playerid, -1, sprintf("[Alexa]: %s offline faction updated to %s [%d] with rank %s", Account, Faction:GetName(factionid), factionid, Faction:GetRankName(factionid, rankid)));
    return 1;
}
