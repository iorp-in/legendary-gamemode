...
hook OnPlayerUpdateEx(playerid) {
    if (GetPlayerCash(playerid) != GetPlayerMoney(playerid)) {
        ResetPlayerMoney(playerid); //Resets the money in the original moneybar, Do not remove!
        GivePlayerMoney(playerid, GetPlayerCash(playerid)); //Sets the money in the moneybar to the serverside cash, Do not remove!
    }
    return 1;
}
