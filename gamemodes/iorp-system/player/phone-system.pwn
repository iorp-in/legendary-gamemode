...
hook OnPlayerLogin(playerid) {
    ...
    rows = cache_num_rows();
    if (rows) cache_get_value_int(0, "PHONE_NUMBER", MobilePlayer[playerid][pNumber]);
    else {
        do MobilePlayer[playerid][pNumber] = RandomEx(80123456, 98765432);
        while (NumberUsed(playerid));
        format(query, sizeof query, "INSERT INTO `phoneNumbers` (`PHONE_NUMBER`, `USERNAME`) VALUES ('%d',\"%s\")", MobilePlayer[playerid][pNumber], GetPlayerNameEx(playerid));
        mysql_tquery(Database, query);
    }
    cache_delete(result);
    return 1;
...
stock NumberUsed(playerid) {
    ...
    if (MobilePlayer[i][pNumber] == MobilePlayer[playerid][pNumber] && i != playerid) return 1;

    new query[148];
    format(query, sizeof query, "SELECT * FROM `phoneNumbers` WHERE `PHONE_NUMBER` = '%d'", MobilePlayer[playerid][pNumber]);
    new rows, Cache:result = mysql_query(Database, query);
    rows = cache_num_rows();
    if (rows) return 1;
    cache_delete(result);
...
hook OnPlayerText(playerid, text[]) {
    if (MobilePlayer[playerid][calling] == 1) {
        ...
        format(str, sizeof(str), "%d (%s): %s", MobilePlayer[playerid][pNumber], str, text);
    ...
    if (MobilePlayer[playerid][calling] == 3) {
        new str[158];
        GetPlayerName(playerid, str, 24);
        format(str, sizeof(str), "New message from %d (%s) MESSAGE:", MobilePlayer[playerid][pNumber], str);
        new str2[158];
...
stock Phone:MakeCall(playerid, phoneNumber) {
    foreach(new i:Player) {
        if (IsPlayerConnected(i)) {
            if (MobilePlayer[i][pNumber] == phoneNumber && phoneNumber != 0 && !MobilePlayer[i][calling] && i != playerid) {
                SendClientMessageEx(i, 0xFFFF00AA, sprintf("%d (%s) is calling you ...", MobilePlayer[playerid][pNumber], GetPlayerNameEx(playerid)));
...
hook OnPlayerClickTextDrawEx(playerid, Text:clickedid) {
    ...
    if (clickedid == Galaxy3KeyBoard[13]) {
        ...
        if (size == 8) {
            ...
            foreach(new i:Player) {
                if (IsPlayerConnected(i)) {
                    if (MobilePlayer[i][pNumber] == phonenumb && phonenumb != 0 && !MobilePlayer[i][calling] && i != playerid) {
...
stock Phone:GetPlayerNumber(playerid) {
    return MobilePlayer[playerid][pNumber];
}
