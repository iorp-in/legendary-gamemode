...
APCP:OnInit(playerid, targetid, page) {
    if (page != 0) return 1;
    if (!NameTagAuth:GetPlayer(targetid)) APCP:AddCommand(playerid, "Enable name tag");
    else APCP:AddCommand(playerid, "Disable name tag");
    return 1;
}

APCP:OnResponse(playerid, targetid, page, response, listitem, const inputtext[]) {
    if (!response || page != 0) return 1;
    if (IsStringSame("Enable name tag", inputtext)) {
    ...
    if (IsStringSame("Disable name tag", inputtext)) {
        NameTagAuth:Command(playerid, targetid);
...
hook OnAlexaResponse(playerid, const cmd[], const text[]) {
    if (GetPlayerAdminLevel(playerid) != 3 || !IsStringSame(text, "name tag mode")) return 1;
