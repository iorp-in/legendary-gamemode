...
hook OnPurchaseFromShop(playerid, shopid, shopItemId) {
    ...
    if (Backpack:isValidBackpack(backPackId)) {
        if (shopItemId == 39 && Backpack:GetInvItemQuantity(backPackId, InvMed_OralAntihistamine) == InvLimit_Oral_Antihistamines) {
            SendClientMessage(playerid, -1, "{4286f4}[Alexa]: {FFFFEE}you don't have enough space in your backpack to purchase this medicine");
            return ~1;
        }
        if (shopItemId == 40 && Backpack:GetInvItemQuantity(backPackId, InvMed_DitropanXL) == InvLimit_Ditropan_XL) {
            SendClientMessage(playerid, -1, "{4286f4}[Alexa]: {FFFFEE}you don't have enough space in your backpack to purchase this medicine");
            return ~1;
        }
        if (shopItemId == 41 && Backpack:GetInvItemQuantity(backPackId, InvMed_Benzodiazepine) == InvLimit_Benzodiazepines) {
            SendClientMessage(playerid, -1, "{4286f4}[Alexa]: {FFFFEE}you don't have enough space in your backpack to purchase this medicine");
            return ~1;
        }
        if (shopItemId == 42 && Backpack:GetInvItemQuantity(backPackId, InvMed_Temazepam) == InvLimit_Temazepam) {
            SendClientMessage(playerid, -1, "{4286f4}[Alexa]: {FFFFEE}you don't have enough space in your backpack to purchase this medicine");
            return ~1;
        }
        if (shopItemId == 43 && Backpack:GetInvItemQuantity(backPackId, InvMed_BismuthSubsalicylate) == InvLimit_Bismuth_Subsalicylate) {
            SendClientMessage(playerid, -1, "{4286f4}[Alexa]: {FFFFEE}you don't have enough space in your backpack to purchase this medicine");
            return ~1;
        }
    }
    ...
    if (Backpack:isValidBackpack(backPackId)) {
        if (shopItemId == 39) Backpack:PushItem(backPackId, InvMed_OralAntihistamine, 1);
        if (shopItemId == 40) Backpack:PushItem(backPackId, InvMed_DitropanXL, 1);
        if (shopItemId == 41) Backpack:PushItem(backPackId, InvMed_Benzodiazepine, 1);
        if (shopItemId == 42) Backpack:PushItem(backPackId, InvMed_Temazepam, 1);
        if (shopItemId == 43) Backpack:PushItem(backPackId, InvMed_BismuthSubsalicylate, 1);
...
FlexDialog:DocMotives(doctorid, response, motiveid, const inputtext[], patientid, const payload[]) {
    ...
    medicineList[1] = InvMed_DitropanXL;
    medicineList[2] = InvMed_Benzodiazepine;
    medicineList[3] = InvMed_Temazepam;
    medicineList[4] = InvMed_BismuthSubsalicylate;
