...
new ElevatorState, ElevatorFloor; // If Idle or Waiting, this is the current floor. If Moving, the floor it's moving to.
...
stock ShowElevatorDialog(playerid) {
    ...
    return FlexPlayerDialog(playerid, "ElevatorMenu", DIALOG_STYLE_LIST, "Elevator", string, "Accept", "Cancel");
}

FlexDialog:ElevatorMenu(playerid, response, listitem, const inputtext[], extraid, const payload[]) {
...
stock Float:GetElevatorZCoordForFloor(floorid) return (GROUND_Z_COORD + FloorZOffsets[floorid] + ELEVATOR_OFFSET); // A small offset for the elevator object itself.
stock Float:GetDoorsZCoordForFloor(floorid) return (GROUND_Z_COORD + FloorZOffsets[floorid]);