...
ASCP:OnInit(playerid, page) {
    if (page != 0) return 1;
    ASCP:AddCommand(playerid, "Enterance");
    return 1;
}

ASCP:OnResponse(playerid, page, response, listitem, const inputtext[]) {
    if (!response || page != 0 || !IsStringSame("Enterance", inputtext)) return 1;
    Entrance:AdminPanel(playerid);
    return ~1;
}

hook OnAlexaResponse(playerid, const cmd[], const text[]) {
    if (!IsPlayerMasterAdmin(playerid) || !IsStringSame(text, "entrance system")) return 1;
    Entrance:AdminPanel(playerid);
    return ~1;
}
