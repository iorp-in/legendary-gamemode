...
ASCP:OnInit(playerid, page) {
    if (page != 0) return 1;
    ASCP:AddCommand(playerid, "Drug");
    return 1;
}

ASCP:OnResponse(playerid, page, response, listitem, const inputtext[]) {
    if (!response || page != 0 || !IsStringSame("Drug", inputtext)) return 1;
    Drug:AdminMenu(playerid);
    return ~1;
}

hook OnAlexaResponse(playerid, const cmd[], const text[]) {
    if (!IsPlayerMasterAdmin(playerid) || !IsStringSame(text, "drug system")) return 1;
    Drug:AdminMenu(playerid);
    return ~1;
}
