...
cmd:getadmins(playerid, const params[]) {
    ...
    if (rows) {
        SendClientMessage(playerid, -1, "[Alexa]: Admin List");
        ...
        while (count < rows) {
            ...
            SendClientMessage(playerid, -1, sprintf("Username: %s Admin Level: %d", uName, level));
            count++;
        }
    } else {
        SendClientMessage(playerid, -1, "[Alexa]: No Admin Found");
    }
...
cmd:ban(playerid, const params[]) {
    ...
    if (sscanf(params, "s[50]ds[100]", Account, mins, banreason)) return SendClientMessage(playerid, -1, "[USAGE]: /ban [Account] [mins] [reason]");
    if (!IsValidAccount(RemoveMalChars(Account))) return SendClientMessage(playerid, -1, "[Alexa]: Account not Found");
    if (mins < 1) return SendClientMessage(playerid, -1, "[Alexa]: Invalid mins for ban");
    if (mins > 43800) return SendClientMessage(playerid, -1, "[Alexa]: can not ban account for more then one month, consider deactivation");
    ...
    SendClientMessage(playerid, -1, sprintf("[Alexa]: %s account has been banned", Account));
...
cmd:unban(playerid, const params[]) {
    if (!IsPlayerMasterAdmin(playerid)) return 0;
    new Account[50];
    if (sscanf(params, "s[50]", Account)) return SendClientMessage(playerid, -1, "[USAGE]: /unban [Account]");
    if (!IsValidAccount(RemoveMalChars(Account))) return SendClientMessage(playerid, -1, "[Alexa]: Account not Found");
    ...
    SendClientMessage(playerid, -1, sprintf("[Alexa]: %s account has been unbanned", Account));
    return 1;
}

cmd:enableaccount(playerid, const params[]) {
    if (!IsPlayerMasterAdmin(playerid)) return 0;
    new Account[50];
    if (sscanf(params, "s[50]", Account)) return SendClientMessage(playerid, -1, "[USAGE]: /enableaccount [Account]");
    if (!IsValidAccount(RemoveMalChars(Account))) return SendClientMessage(playerid, -1, "[Alexa]: Account not Found");
    mysql_tquery(Database, sprintf("UPDATE `players` SET Status = true WHERE `Username` = \"%s\" LIMIT 1", Account));
    SendClientMessage(playerid, -1, sprintf("[Alexa]: %s account has been activated", Account));
    CallRemoteFunction("OnAccountEnabled", "s", Account);
    return 1;
}

cmd:disableaccount(playerid, const params[]) {
    if (!IsPlayerMasterAdmin(playerid)) return 0;
    new Account[50], disablereason[100];
    if (sscanf(params, "s[50]s[100]", Account, disablereason)) return SendClientMessage(playerid, -1, "[USAGE]: /disableaccount [Account] [Reason]");
    if (!IsValidAccount(RemoveMalChars(Account))) return SendClientMessage(playerid, -1, "[Alexa]: Account not Found");
    ...
    SendClientMessage(playerid, -1, sprintf("[Alexa]: %s account has been deactivated", Account));
...
cmd:updatename(playerid, const params[]) {
    if (!IsPlayerMasterAdmin(playerid)) return 0;
    new CurrentName[30], NewName[30];
    if (sscanf(params, "s[30]s[30]", CurrentName, NewName)) return SendClientMessage(playerid, -1, "[USAGE]: /updatename [account name] [new account name]");
    if (!IsValidAccount(RemoveMalChars(CurrentName))) return SendClientMessage(playerid, -1, "[Alexa]: Account not Found");
    if (IsValidAccount(RemoveMalChars(NewName))) return SendClientMessage(playerid, -1, "[Alexa]: new name is occupied, can't update name request.");
    ...
    SendClientMessage(playerid, -1, sprintf("[Alexa]: %s account has been renamed to %s", CurrentName, NewName));
    return 1;
}

cmd:getmasteradmins(playerid, const params[]) {
    if (!IsPlayerMasterAdmin(playerid)) return 0;
    new Cache:mysql_cache = mysql_query(Database, "SELECT * FROM `players` WHERE MasterAdmin = true");
    new rows = cache_num_rows();
    if (rows) {
        SendClientMessage(playerid, -1, "[Alexa]: Master Admin List");
        new uName[50], count;
        while (count < rows) {
            cache_get_value_name(count, "username", uName, sizeof(uName));
            SendClientMessage(playerid, -1, sprintf("Username: %s", uName));
            count++;
        }
    } else {
        SendClientMessage(playerid, -1, "[Alexa]: No Master Admin Found");
    }
    cache_delete(mysql_cache);
    return 1;
}
