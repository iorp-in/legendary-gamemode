...
hook OnAlexaResponse(playerid, const cmd[], const text[]) {
    if (!IsPlayerMasterAdmin(playerid) || !IsStringSame(text, "bank system")) return 1;
    Bank:AdminShowMenu(playerid);
    return ~1;
}
...
cmd:setbankpassword(playerid, const params[]) {
    if (!IsPlayerMasterAdmin(playerid)) return 0;
    new accountid, newpassword[100];
    if (sscanf(params, "ds[100]", accountid, newpassword)) return SendClientMessage(playerid, -1, "[USAGE]: /setbankpassword [AccountID] [new password]");
    if (!Bank:IsAccountAcitve(accountid)) return SendClientMessage(playerid, -1, "[ERROR]: Invalid bank account id.");
    new query[512];
    ...
    mysql_tquery(Database, query);
    SendClientMessage(playerid, -1, sprintf("Account ID %d password has been set to '%s'.", accountid, newpassword));
    return 1;
}

cmd:enablebankaccount(playerid, const params[]) {
    if (!IsPlayerMasterAdmin(playerid)) return 0;
    new accountid;
    if (sscanf(params, "d", accountid)) return SendClientMessage(playerid, -1, "[USAGE]: /enablebankaccount [AccountID]");
    if (!Bank:IsValidAccountID(accountid)) return SendClientMessage(playerid, -1, "[ERROR]: Invalid bank account id.");
    mysql_tquery(Database, sprintf("UPDATE bankAccounts SET Disabled = 0 WHERE ID=%d", accountid));
    SendClientMessage(playerid, -1, sprintf("Account ID %d has been enabled.", accountid));
    return 1;
}

cmd:disablebankaccount(playerid, const params[]) {
    if (!IsPlayerMasterAdmin(playerid)) return 0;
    new accountid;
    if (sscanf(params, "d", accountid)) return SendClientMessage(playerid, -1, "[USAGE]: /disablebankaccount [AccountID]");
    if (!Bank:IsValidAccountID(accountid)) return SendClientMessage(playerid, -1, "[ERROR]: Invalid bank account id.");
    mysql_tquery(Database, sprintf("UPDATE bankAccounts SET Disabled = 1 WHERE ID=%d", accountid));
    SendClientMessage(playerid, -1, sprintf("Account ID %d has been disabled.", accountid));
    return 1;
}
